...
1. **[ESP32 Controller (`/automation`)](./automation/README.md)**
   The core hardware controller built on the ESP32 platform. It handles all real-time sensor monitoring (water temperature, ambient temperature, and humidity), equipment scheduling (wave makers, lights, air pumps), physical overrides and serves the local web-based control dashboard.
...
- **Configuration & Firmware Management:** Built-in Backup/Restore functionality (with version validation and auto-reboot), custom NTP time synchronization paired with a robust DS1307 hardware RTC for persistent timekeeping, scheduled firmware updates, and robust Over-The-Air (OTA) update support with seamless UI state management and an automatic rollback mechanism on boot failures.
...
- **High Stability:** Optimized FreeRTOS task scheduling ensures reliable long-term execution, prevents Watchdog Timer (WDT) resets, and utilizes staggered relay initialization to prevent power surges during startup.
